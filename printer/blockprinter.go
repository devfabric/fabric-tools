...
type Printer interface {
	// PrintBlockchainInfo 打印当前区块信息
	PrintBlockchainInfo(info *types.BlockChainInfo)
	...
	// PrintChaincodes 打印链码信息
	PrintChaincodes(chaincodes []*types.ChainCodeInfo)
...
func (p *BlockPrinter) PrintBlockchainInfo(info *types.BlockChainInfo) {
	if p.Formatter == nil {
		fmt.Printf("%s\n", info)
		return
	}
...
func (p *BlockPrinter) PrintPeers(peers []*types.Peer) {
	if p.Formatter == nil {
		fmt.Printf("%s\n", peers)
		return
	}
...
func (p *BlockPrinter) PrintChaincodes(chaincodes []*types.ChainCodeInfo) {
	if p.Formatter == nil {
		fmt.Printf("%s\n", chaincodes)
		return
	}
...
func (p *BlockPrinter) PrintTxProposalResponse(response *fab.TransactionProposalResponse, payloadOnly bool) {
	if payloadOnly {
		if response.Status != http.StatusOK {
			...
			// p.FieldDecode("Payload", response.ProposalResponse.Response.Payload)
...
func (p *BlockPrinter) PrintChaincodeInfo(ccInfo *types.ChainCodeInfo) {
...
func (p *BlockPrinter) PrintPeer(peer *types.Peer) {
	p.Field("URL", peer.URL)
	p.Field("MSP", peer.MSP)
	p.Field("BlockHeight", peer.BlockHeight)
}
